...
local anchorSystemMixin = {
    ...
    CreateScreenAnchor = function(self, anchorKey, frameName, growDirection, setupFunction, sortFunction, frameCreateFunction)
        ...
        anchorFrame.framePool:SetCallbackOnRelease(function(frame) frame:StopTimer(); frame:Hide() end)
        anchorFrame.framePool:SetCallbackOnReleaseAll(function(frame) frame:StopTimer(); frame:Hide() end)
        anchorFrame.framePool:SetSortFunction(function(a, b) return anchorFrame.sortFunction(a, b) end)
    ...
    Reorder = function(self, anchorFrame)
        ...
        for i = 1, #allComponentsShown do
            ...
            if (lastComponent) then
                ...
                if growDir == "top" then
                    thisComponent:SetPoint("bottom", lastComponent.widget or lastComponent, "top", 0, 0)
                elseif growDir == "bottom" then
                    thisComponent:SetPoint("top", lastComponent.widget or lastComponent, "bottom", 0, 0)
                elseif growDir == "left" then
                    thisComponent:SetPoint("right", lastComponent.widget or lastComponent, "left", 0, 0)
                elseif growDir == "right" then
                    thisComponent:SetPoint("left", lastComponent.widget or lastComponent, "right", 0, 0)
                end
            else
